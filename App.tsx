               ...
               <div className="mt-4 animate-fade-in">
                  ...
                  <p className="font-sans text-lg md:text-xl text-pa-black max-w-lg mb-10 leading-relaxed font-medium" style={{ fontSize: '16px' }}>
                    {HERO_SLIDES[currentSlide].subheadline.split(/(\[.*?\])/g).map((part, idx) =>
                      part.match(/\[.*?\]/) ? (
                        <span key={idx} className="font-bold italic" style={{ fontSize: '16px' }}>{part}</span>
                      ) : (
                        <span key={idx} style={{ fontSize: '16px' }}>{part}</span>
                      )
                    )}
                  </p>
        ...
        <div className={`${CARD_STYLE} ${CARD_PADDING} bg-stone-50`}>
            ...
            {SERVICES.map((service) => (
                <div key={service.id} className="flex flex-col items-center">
                    <div
                        className={`${expandedService === service.id ? 'bg-pa-sage' : 'bg-white'} p-6 md:p-8 border-2 border-pa-black rounded-xl flex flex-col shadow-[12px_12px_0px_0px_#1a1a1a] transition-colors w-full`}
                    >
                    ...
                    {/* Squiggly arrow pointing down for selected service */}
                    {expandedService === service.id && (
                        <svg width="40" height="50" viewBox="0 0 40 50" className="mt-4 text-pa-black animate-bounce">
                            <path
                                d="M20 0 Q25 10, 15 15 Q5 20, 20 25 Q35 30, 15 35 Q5 40, 20 50"
                                fill="none"
                                stroke="currentColor"
                                strokeWidth="3"
                                strokeLinecap="round"
                    ...
                    <div className="lg:col-span-4 flex flex-col gap-2">
                        {USE_CASES.map((uc) => (
                            <button
                                ...
                                className={`text-left p-4 rounded-lg border-2 transition-all flex items-center justify-between group ${
                                    activeTab === uc.id
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '17px', fontWeight: 500, color: '#1a1a1a', fontStyle: 'normal' }}>{uc.title}</span>
                                </div>
